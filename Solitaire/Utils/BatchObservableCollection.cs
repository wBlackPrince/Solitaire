...
// расширенная версия ObservableCollection<T> которая умеет объединять несколько изменений
// коллекции в одно уведомление
// защищает от изменения коллекции во время обработки событий

// Collection<T> - базовая коллекция
//  INotifyCollectionChanged - сообщает UI об изменениях
// INotifyPropertyChanged - сообщает об изменении Count, Item[]
public sealed class BatchObservableCollection<T>: Collection<T>, INotifyCollectionChanged, INotifyPropertyChanged
{
    ...
    // есть два режима работы объекта
    // _notifyInfo == null   - rootCollection, обычный хранит: реально хранит данные, вызывает события, используется UI
    // _notifyInfo != null   - не хранит данные, работает с Items родителя, перехватывает события, накапливает информацию
    private readonly NotificationInfo? _notifyInfo;
    
    // включаются при первой подписке
    ...
    private BatchObservableCollection(BatchObservableCollection<T>? parent, bool notify): base(parent?.Items!)
    {
            ...
            if (null == _notifyInfo)
            {
                if (null == PropertyChanged)
                {
                    _fireCountAndIndexerChanged = delegate
                    {
                        ...
                        OnPropertyChanged(new PropertyChangedEventArgs(IndexName));
                    };
                    _fireIndexerChanged = delegate { OnPropertyChanged(new PropertyChangedEventArgs(IndexName)); };
                }
    ...
    {
        // если во время уведомления попытаются изменить коллекцию, будет вызвано исключение
    ...
    private void DisposeInternal()
    {
        ...
        if (null != _notifyInfo.RootCollection?.PropertyChanged)
        {
            ...
            _notifyInfo.RootCollection.OnPropertyChanged(new PropertyChangedEventArgs(IndexName));
        }
    ...
    // защищает коллекцию от повторного изменения во время рассылки уведомлений
    private class ReentryMonitor : IDisposable
    {
        private int _referenceCount;

        // вызывается перед рассылкой
    ...
    // этот класс собирает изменения в одно событие
    private class NotificationInfo
    {
            ...
            return (sender, args) =>
            {
                ...
                switch (_action)
                {
                    ...
                    case NotifyCollectionChangedAction.Remove:
                        ...
                        IsCountChanged = true;
                        wrapper.CollectionChanged = (_action, e) =>
                        {
                            AssertActionType(e);
        ...
        // проверка на то что операции не смешиваются
